...
// turn on the LED if the distance is between dist_min and dist_max
  ...
  else if(100.0 <= dist_raw && dist_raw < 150.0){
        for (int fadeValue = 255 ; fadeValue >= 127; fadeValue -= 3) {
            analogWrite(PIN_LED, fadeValue);
         }
    }
  else if(150.0 <= dist_raw && dist_raw < 200.0){
        for (int fadeValue = 127 ; fadeValue >= 0; fadeValue -= 3) {
            analogWrite(PIN_LED, fadeValue);
      }
    }
  else if(200.0 <= dist_raw && dist_raw < 250.0){
        for (int fadeValue = 0 ; fadeValue <= 127; fadeValue += 3) {
            analogWrite(PIN_LED, fadeValue);
      }
    }
  else if(250.0 <= dist_raw && dist_raw <= 300.0){
        for (int fadeValue = 127 ; fadeValue <= 255; fadeValue += 3) {
            analogWrite(PIN_LED, fadeValue);
      }
    }
