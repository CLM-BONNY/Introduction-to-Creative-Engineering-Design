...
// adjust servo position according to the USS read value
  ...
  else if (180 <= dist_raw && dist_raw < 270) {
      for (int servo = _DUTY_MIN; servo <= _DUTY_NEU ; servo ++)
          myservo.writeMicroseconds(servo);
  }
  else if (270 <= dist_raw && dist_raw < 360) {
      for (int servo = _DUTY_NEU; servo <= _DUTY_MAX ; servo ++)
          myservo.writeMicroseconds(servo);
  }
  else if (360 <= dist_raw) myservo.writeMicroseconds(_DUTY_MAX);
